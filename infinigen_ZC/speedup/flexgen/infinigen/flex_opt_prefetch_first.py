...
from flexgen.utils import (Task, ExecutionEnv, GB, T, ValueHolder,
    ...
    # Get absolute path to infinigen
infinigen_path = os.path.abspath("/home/ipa/dongheon/infinigen_ZC/speedup/infinigen/infinigen")
...
gather_copy_path = os.path.abspath("/home/ipa/dongheon/infinigen_ZC/speedup/scripts/figure14/kernel_test")
sys.path.append(gather_copy_path)
...
class SelfAttention:
    ...
    def init_cache_one_gpu_batch(self, cache_home):
        ...
        nvtx.range_push("InitCacheHome")
        #print("device: ", device)
        cache = device.init_cache_one_gpu_batch(self.config, self.task, self.policy)
        #print("cache[0].data is pinned? : ", cache[0].data.is_pinned())
    ...
    def prefetch_cache(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        ...
        k_home, v_home = cache_home.val
        #print(f"k_home type: {type(k_home)}, shape: {k_home.shape}")
        #print(f"v_home type: {type(v_home)}, shape: {v_home.shape}")
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                nvtx.range_push("select_kv")
                #print(f"{prefetch_idx.shape[0]}")
                ...
                nvtx.range_push("smart_copy")
                #print(f"k_c shape: {k_c.shape}, v_c shape: {v_c.shape}, indices shape: {indices[0].stop}")
    ...
    def prefetch_cache_direct(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        if i == 0:  # prefill, no cache to prefetch
            return
        # Retrieve key and value caches.
        k_home, v_home = cache_home.val  
        # We assume that k_home and v_home have shape (n, b*h, d)
        ...
        if path == 0:  # Direct copy using our gather_bf16 kernel
            if self.policy.attn_sparsity >= 1.0:
                # Determine dimensions.
                nprime = prefetch_idx.shape[0]          # number of prefetch rows
                n    = k_home.shape[0]                    # number of tokens in the source cache
                bh   = k_home.shape[1]                    # b*h dimension
                d    = k_home.shape[2]                    # feature dimension

                new_b = self.policy.gpu_batch_size     # new_b = b*h
                new_h = 32     # new_h = 1, so new_b * new_h = b*h

                # Allocate new GPU destination tensors for keys and values, with shape (nprime, b*h, d)
                k_dest = torch.empty((nprime, bh, d), dtype=k_home.dtype, device='cuda')
                v_dest = torch.empty((nprime, bh, d), dtype=v_home.dtype, device='cuda')

                # --- Launch the gather kernel for the key cache ---
                nvtx.range_push("gather_keys")
                k_src_ptr = int(k_home.data.data_ptr())
                
                k_dest_ptr = int(k_dest.data_ptr())
                prefetch_ptr = int(prefetch_idx.data_ptr())
      

                gather_copy.gather_bf16_optimized(k_src_ptr, k_dest_ptr, prefetch_ptr, n, nprime, new_b, new_h, d, 16, 128)
                nvtx.range_pop()  # End gather_keys

                # --- Launch the gather kernel for the value cache ---
                nvtx.range_push("gather_values")
                v_src_ptr = int(v_home.data.data_ptr())
                v_dest_ptr = int(v_dest.data_ptr())

                gather_copy.gather_bf16_optimized(v_src_ptr, v_dest_ptr, prefetch_ptr, n, nprime, new_b, new_h, d, 16, 128)
                nvtx.range_pop()  # End gather_values

                # --- Wrap the gathered GPU tensors into TorchTensor objects ---
                k_c = ( TorchTensor((nprime, bh, d), k_home.dtype, k_dest, dst), True)
                v_c = ( TorchTensor((nprime, bh, d), v_home.dtype, v_dest, dst), True)
                
                # --- Store the wrapped tensors in the cache read buffer ---
                nvtx.range_push("store_copy")
                
                cache_read_buf.store((k_c, v_c))
                nvtx.range_pop()
            else:
                raise ValueError("Unsupported attn_sparsity setting for gather-based prefetch.")
        elif path == 1 or path == 2:
            raise ValueError(f"Not implemented path: {path}")
        else:
            raise ValueError(f"Invalid path: {path}")
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, j, k, prefetch_cache_func, warmup, partial_weight_read_buf, partial_cache_read_buf, speculation_stream, prev_partial_cache_read_buf, prev_partial_weight_read_buf, weight_home, gather_worker, attn_layer):
        ...
        if i == 0:  # prefill
            ...
            #print(f"cache_read_buf.pop() class: {type(cache_read_buf.pop())}, value: {cache_read_buf.pop()}")
            (k_cache, donate[12]), (v_cache, donate[13]) = cache_read_buf.pop()
            if self.enable_prefetching:
                partial_k_cache = partial_cache_read_buf.val
            if self.enable_prefetching:
                h, new_k_cache, new_v_cache, self.prefetch_idx, future = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, p_w_q, partial_k_cache, speculation_stream, self.alpha, self.max_num_kv, i, j, k, prefetch_cache_func, gather_worker, attn_layer)
            else:
                h, new_k_cache, new_v_cache, _ , future = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, None, None, None, None, None, None, None, None, None, None, None)
            cache_write_buf.store((new_k_cache, new_v_cache))
...
class MLP:
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        ...
        if k == 0:
            ...
            # print(f"w_ln shape: {w_ln.shape}")
            # print(f"b_ln shape: {b_ln.shape}")
...
class OptLM:
    def __init__(self,
        ...
        self.gather_worker = concurrent.futures.ThreadPoolExecutor(max_workers=1)
    ...
    def load_weight(self, i, j, k, overlap=True):
        ...
        if overlap:
            with torch.cuda.stream(self.load_weight_stream):
                nvtx.range_push("Load_Weight_with_Stream")
                self.layers[j].load_weight(self.weight_home[j], self.weight_read_buf[j], k)
                nvtx.range_pop()
        else:
            nvtx.range_push("Load_Weight_without_Stream")
            self.layers[j].load_weight(self.weight_home[j], self.weight_read_buf[j], k)
            nvtx.range_pop()
        ...
        if overlap:
            with torch.cuda.stream(self.load_cache_stream):
                if j not in self.attn_layer[2:]:
                    nvtx.range_push("Load_Cache_with_Stream")
                    self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i)
                    nvtx.range_pop()
                
        else:
            if j not in self.attn_layer[2:]:
                nvtx.range_push("Load_Cache_without_Stream")
                self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i)
                nvtx.range_pop()
    
    def prefetch_cache(self, prefetch_idx, i, j, k, speculation_stream,overlap=True):
        # Handle corner cases
        if i == 0:  # prefill, no cache
            return
        if k == self.num_gpu_batches:
        ...
        speculation_stream.synchronize()
        next_attn = self.attn_layer[self.attn_layer.index(j) + 1]

        #print(f"Prefetch_Cache_i{i}_j{j}_k{k}")
        nvtx.range_push(f"Prefetch_Cache_i{i}_j{j}_k{k}")
        with torch.cuda.stream(self.prefetch_cache_stream):
            self.layers[next_attn].prefetch_cache_direct(self.cache_home[next_attn][k], self.cache_read_buf[next_attn][k], i, prefetch_idx, self.prefetch_cache_stream)
        nvtx.range_pop()
    ...
    def store_cache(self, i, j, k, overlap=True):
        ...
        if overlap:
            ...
            nvtx.range_push("Store_Cache_without_Stream")
            self.layers[j].store_cache(self.cache_home[j][k], self.cache_write_buf[j][k], i)
            nvtx.range_pop()
    ...
    def store_hidden(self, i, j, k):
        ...
        nvtx.range_push("Store_Hidden")
        if j == self.num_layers - 1:  # store to output
            gpu_batch_size = self.policy.gpu_batch_size
            left, right = k * gpu_batch_size, (k + 1) * gpu_batch_size
            ids = self.hidden[i][j][k].pop().data.detach().cpu().numpy()
            #print(f"i: {i}, j: {j}, k: {k}, ids: {ids}")
            pos = self.task.prompt_len + i
            if self.task.stop:
        ...
        if isinstance(self.layers[j], SelfAttention) or isinstance(self.layers[j], TransformerLayer):
            if k==0: # Generate skewing matrix for first input of warmup
                ...
                if j in self.attn_layer[2:]:
                    ...
                    self.gather_future = self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                        self.weight_read_buf[j], self.attention_mask[k],
                        self.cache_write_buf[j][k], i, j, k, self.prefetch_cache, warmup_state, self.partial_weight_read_buf[j], self.partial_cache_read_buf[j][k], self.speculation_stream,
                        self.partial_cache_read_buf[prev_attn][k], self.partial_weight_read_buf[prev_attn], self.weight_home[j], self.gather_worker, self.attn_layer)
                else:
                    self.gather_future = self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                        self.weight_read_buf[j], self.attention_mask[k],
                        self.cache_write_buf[j][k], i, j, k, self.prefetch_cache, warmup_state, self.partial_weight_read_buf[j], self.partial_cache_read_buf[j][k], self.speculation_stream,
                        None, None, self.weight_home[j], self.gather_worker, self.attn_layer)
        else:
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    ...
                    if (j in self.attn_layer[1:-1]) and (i > 0):
                        if(self.gather_future != None):
                            self.gather_future.result()
                            self.gather_future = None
                        self.sync()
                        nvtx.range_pop()
...
def run_flexgen(args):
    ...
    #print(f"sep_layer: {args.sep_layer}")
    ...
    output_only = tokenizer.batch_decode(output_ids[0][1920:], skip_special_tokens=True)
    show_str = "Outputs_only:\n" + 70 * '-' + "\n"
    #print(show_str)
    print(output_only)
