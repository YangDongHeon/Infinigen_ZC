...
infinigen_path = os.path.abspath("/home/ipa/dongheon/infinigen_DH/speedups/infinigen/infinigen")
    ...
    def prefetch_cache_direct(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        ...
        if path == 0:  # Direct copy using our gather_bf16 kernel
            if self.policy.attn_sparsity >= 1.0:
                ...
                gather_copy.gather_bf16_optimized(k_src_ptr, k_dest_ptr, prefetch_ptr, n, nprime, new_b, new_h, d, 32, 512)
                ...
                gather_copy.gather_bf16_optimized(v_src_ptr, v_dest_ptr, prefetch_ptr, n, nprime, new_b, new_h, d, 32, 512)
        ...
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    ...
                    if j in [x + 1 for x in self.attn_layer[1:-1]] and (i > 0):
                        nvtx.range_push("mlp+prefetch")
                    
                    self.compute_layer(i, j, k)
                    if j in [x + 1 for x in self.attn_layer[1:-1]] and (i > 0):
                        self.sync()
                        timers("prefetch").start()
                        self.prefetch_cache(i, j-1, k, overlap=True)
                        
                    self.sync()
                    
                    if j in [x + 1 for x in self.attn_layer[1:-1]] and (i > 0):
                        timers("prefetch").stop()
                        nvtx.range_pop()
...
def run_flexgen(args):
    ...
    try:
        ...
        prefetch_costs = timers("prefetch").costs
    finally:
        env.close_copy_threads()

    # Log output
    prefetch_latency = sum(prefetch_costs)
    ...
    ideal_decode_latency = decode_latency - prefetch_latency
    ...
    print("+++++++++++++++++++++++++++++++++++++++++++++++++")
    print("Total: " + str(total_latency) + 
          " Prefill: " + str(prefill_latency) + 
          " Decode: " + str(decode_latency) + 
          " Prefetch: " + str(prefetch_latency) +
          " Ideal Decode: " + str(ideal_decode_latency))
    print("=================================================")
