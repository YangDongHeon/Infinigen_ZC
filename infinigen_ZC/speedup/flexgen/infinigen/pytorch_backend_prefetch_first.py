...
#gather_copy_path = os.path.abspath("/home/ipa/dongheon/infinigen_DH/speedups/scripts/figure14/kernel_test")
...
class TorchDevice:
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        ...
        # Print the values only once
        
        #print(f"num_head: {num_head}, hidden_size: {hidden_size}, prompt_len: {prompt_len}, gen_len: {gen_len}, gpu_batch_size: {gpu_batch_size}")
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
                ...
                attn_sparsity, compress_cache, comp_config, p_w_q, partial_k_cache, speculation_stream, alpha, max_num_kv, i, j, k_num, prefetch_cache_func, gather_worker, attn_layer):
        ...
        if p_w_q is not None:
            with torch.cuda.stream(speculation_stream):
                nvtx.range_push("speculate_attention")
        ...
        k = F.linear(new_h, w_k.data, bias=None)
        v = F.linear(hidden, w_v.data, bias=b_v.data)
        if j is not None and j in attn_layer[1:-1] and (i>0):
            nvtx.range_push("Prefetch+Attn+MLP")
            #future = gather_worker.submit(prefetch_cache_func, prefetch_idx, i, j, k, speculation_stream, True)
            prefetch_cache_func(prefetch_idx, i, j, k_num, speculation_stream, True)
        ...
        return TorchTensor.create_from_torch(value, self), k_new, v_new, prefetch_idx, future
