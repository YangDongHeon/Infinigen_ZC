...
def get_opt_config(name, **kwargs):
    CUSTOM_MODEL_BASE_PATH = "/local/dongheon/"

    if "/" in name:
        name = name.split("/")[1]
    name = name.lower()

    resolved_path = os.path.join(CUSTOM_MODEL_BASE_PATH, name)
    ...
    if arch_name == "opt-125m":
        ...
        #print(name)
        config = OptConfig(name=resolved_path,
            max_seq_len=2048, num_hidden_layers=32, n_head=32,
...
def download_opt_weights(model_name, path):
    ...
    if os.path.isabs(model_name) or model_name.startswith("./") or model_name.startswith("../"):
        ...
        if not os.path.exists(model_name):
            raise ValueError(f"Local model path does not exist: {model_name}")
