...
class SelfAttention:
    ...
    def prefetch_cache(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream):
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                nvtx.range_push("gather_cpu")
                #print(f"prefetch_idx: {prefetch_idx}")
                ...
                #print("cache_read_buf.val[0][0].data[0].shape\n",cache_read_buf.val[0][0].data[0].shape)
                #print("cache_read_buf.val[0][0].data[0]\n",cache_read_buf.val[0][0].data[0])
                #print("k_home.data[prefetch_idx[0][0][1]]\n",k_home.data[prefetch_idx[0][0][1]])
    ...
    def prefetch_cache_thread(self, cache_home, cache_read_buf, i, prefetch_idx, prefetch_cache_stream, gather_worker):
        if i == 0:  # prefill, no cache
            return
        ...
        if path == 0:  # Direct copy
            ...
            if self.policy.attn_sparsity >= 1.0:
                # Wrap the inner prefetch work in a thread
                def thread_func():
                    nvtx.range_push("Prefetch_kv")
                    # Run the select_kv which may use F.embedding under the hood
                    self.prefetch_kv.data[0, :prefetch_idx.shape[0]], \
                    self.prefetch_kv.data[1, :prefetch_idx.shape[0]] = select_kv(prefetch_idx, k_home.data, v_home.data)
                    ...
                    with torch.cuda.stream(prefetch_cache_stream):
                        # The result of smart_copy is returned from store, if any
                        return cache_read_buf.store((
                            k_c.smart_copy(dst, indices),
                            v_c.smart_copy(dst, indices),
                        ))
                    
                # Use a ThreadPoolExecutor to run thread_func asynchronously.
                
                future = gather_worker.submit(thread_func)
                return future
        elif path == 1 or path == 2:
            raise ValueError(f"Not implemented path: {path}")
        else:    
        ...
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    ...
                    if (j in self.attn_layer[1:]) and (i > 0):
