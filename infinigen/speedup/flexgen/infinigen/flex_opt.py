        ...
        if i == 0:  # prefill
            ...
            if self.enable_prefetching:
                h, new_k_cache, new_v_cache, self.prefetch_idx = self.compute.mha_gen(h, mask, w_q,
                    ...
                    self.policy.compress_cache, self.policy.comp_cache_config, p_w_q, partial_k_cache, speculation_stream, self.alpha, self.max_num_kv, timers("attention"))
            else:
                h, new_k_cache, new_v_cache, _ = self.compute.mha_gen(h, mask, w_q,
                    b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                    k_cache, v_cache, donate, self.policy.attn_sparsity,
                    self.policy.compress_cache, self.policy.comp_cache_config, None, None, None, None, None, timers("attention"))
            cache_write_buf.store((new_k_cache, new_v_cache))
...
class OptLM:
    ...
    def prefetch_cache(self, i, j, k, overlap=True):
        ...
        nvtx.range_push("Prefetch+MLP")
        self.flag = True

        # Load from cache_home to cache_read_buf
        #print(f"Prefetch_Cache_i{i}_j{j}_k{k}")
        nvtx.range_push(f"Prefetch_Cache_i{i}_j{j}_k{k}")
        self.gather_future = self.layers[next_attn].prefetch_cache(self.cache_home[next_attn][k], self.cache_read_buf[next_attn][k], i, prefetch_idx, self.prefetch_cache_stream)
        nvtx.range_pop()
...
def run_flexgen(args):
    ...
    try:
        ...
        timers("attention").reset()
        timers("speculate_attention").reset()
        timers("gather_cpu").reset()
        output_ids = model.generate(
            inputs, max_new_tokens=args.gen_len,
            debug_mode=args.debug_mode, cut_gen_len=cut_gen_len, verbose=args.verbose)
        nvtx.range_pop()
        costs = timers("generate").costs
        attention_costs = timers("attention").costs
        speculate_attention_costs = timers("speculate_attention").costs
        gather_cpu_costs = timers("gather_cpu").costs
    finally:
        env.close_copy_threads()

    # Log output
    attention_latency = sum(attention_costs)
    speculate_attention_latency = sum(speculate_attention_costs)
    gather_cpu_latency = sum(gather_cpu_costs)
    ...
    print("Total: " + str(total_latency) + 
          ...
          " Attention: " + str(attention_latency) +
          " Speculate: " + str(speculate_attention_latency) +
          " Gather_cpu: " + str(gather_cpu_latency))
    print("=================================================")
